...
# S8 - Save Tuned LSTM Model
import joblib
scaler_path = os.path.join(config.MODELS_PATH, 'lstm_scaler.pkl')
joblib.dump(scaler, scaler_path)
print(f"Scaler saved: {scaler_path}")

import json
params_path = os.path.join(config.MODELS_PATH, 'lstm_params.json')
...
preds_path = os.path.join(config.MODELS_PATH, 'lstm_predictions.csv')
preds_out.to_csv(preds_path)
print(f"LSTM predictions saved: {preds_path} | rows: {len(preds_out)}")
...
for seq_len in [14, 60]:
    ...
    # Hyperopt
    def objective(params):
        m = LSTMForecaster(hidden_size=int(params['hidden_size'])).to(device)
    ...
    final_model = LSTMForecaster(hidden_size=tuned_params['hidden_size']).to(device)
    ...
    print(f"LSTM seq{seq_len} Tuned -- RMSE: {rmse:.2f} | MAE: {mae:.2f} | MAPE: {mape:.2f}% | R²: {r2:.4f} | Bias: {bias:.2f}")

    with mlflow.start_run(run_name=f"lstm-seq{seq_len}-tuned"):
        mlflow.log_params(tuned_params)
        mlflow.log_metrics({"rmse": rmse, "mae": mae, "mape": mape, "r2": r2, "bias": bias})
...
# - RMSE vs SARIMAX baseline (185.90):
# - GPU training time approx: 3.10
